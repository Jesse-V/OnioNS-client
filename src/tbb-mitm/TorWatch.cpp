...
{  // adapted from wait's manpage
  ...
  do
  {
    ...
    if (WIFEXITED(status))
      std::cout << "Tor exited, status= " << WEXITSTATUS(status) << std::endl;
    else if (WIFSIGNALED(status))
      std::cout << "Tor killed by signal " << WTERMSIG(status) << std::endl;
    else if (WIFSTOPPED(status))
      std::cout << "Tor stopped by signal " << WSTOPSIG(status) << std::endl;
    else if (WIFCONTINUED(status))
      std::cout << "Tor continued\n";
